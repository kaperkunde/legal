# plek.je Acceptable Use Policy

**Effective date:** 12 June 2026  
**Provider:** Kaperkunde, operating as **[plek.je](https://plek.je)**  
**Website:** [https://plek.je](https://plek.je)  
**KVK number:** 42061326  
**Address:** Prof. E.M. Meijerslaan 1, Amstelveen, The Netherlands  
**Support:** support@plek.je  
**Abuse reports:** abuse@plek.je  
**Privacy/GDPR requests:** privacy@kaperkun.de  
**Legal notices:** legal@plek.je  

This Acceptable Use Policy (“**AUP**”) forms part of the agreement between you and plek.je. It is incorporated by reference in the [Terms of Service](tos.md).

If this AUP conflicts with a custom written agreement between you and plek.je, the custom agreement controls only where it clearly says so.

---

## 1. Lawful use

You may use the Service only for lawful purposes and in compliance with this AUP, the Terms of Service, applicable law, and generally accepted practices for hosting and publishing online.

The Service is intended for standard website and blog hosting, including personal blogs, business sites, newsletters, membership sites, documentation, small communities, portfolios, and low-volume ecommerce or lead-generation pages where legally compliant.

---

## 2. Prohibited content and activity

You may not use the Service for content or activity that is illegal, harmful, abusive, deceptive, or damaging to plek.je, other customers, third parties, or infrastructure.

Prohibited uses include:

- Illegal content or illegal services
- Malware, phishing, credential theft, or other deceptive activity
- Spam, bulk unsolicited messaging, newsletter abuse, or abusive email practices
- Copyright, trademark, privacy, publicity, or other rights infringement
- Sexual exploitation content, including any content involving minors
- Hateful, abusive, harassing, threatening, or violent content
- Fraud, scams, crypto scams, investment scams, or deceptive schemes
- Illegal marketplaces or sale of illegal goods or services
- Gambling without all required licenses and compliance
- Regulated medical, legal, financial, or other professional services without your own required compliance
- Resource abuse, including excessive CPU, storage, bandwidth, scraping, bot activity, denial-of-service activity, automated misuse, credential stuffing, or other use that degrades or threatens shared infrastructure
- Attempting unauthorized access to the Service, related systems, networks, or other customers’ environments
- Any activity intended to bypass security, usage limits, billing, or service restrictions

---

## 3. Email and member imports

Where the hosted platform supports newsletters or memberships:

- You set up and maintain your own relationship with your email provider (such as Mailgun).
- You must comply with your email provider’s terms, applicable anti-spam rules, ePrivacy requirements, unsubscribe rules, sender-identity rules, and email-marketing law.
- Newsletter content and sending practices are governed by your agreement with your email provider, not by plek.je.
- You must have lawful consent for all members and newsletter recipients, including members imported by CSV, API, or third-party integration.
- You must maintain records demonstrating valid consent where required and provide them on reasonable request.
- You must not import purchased, rented, or scraped email lists.

---

## 4. Uses requiring prior written approval

The following may be allowed only with plek.je’s prior written approval:

- High-traffic scraping, crawling, or automation
- Political campaigning, election-related services, or political content likely to create legal, advertising, donation, targeting, or moderation obligations
- Content likely to create frequent abuse complaints or moderation burden
- Unusual storage, bandwidth, or CPU-heavy workloads

---

## 5. Examples

These examples are illustrative only and do not change the rules above.

**Generally allowed:** personal blogs, business websites, newsletters, membership sites, documentation sites, small community sites, portfolio sites, and low-volume ecommerce or lead-generation pages where legally compliant.

**Not allowed:** phishing, malware, spam, botnets, credential stuffing, copyright piracy, illegal marketplaces, sexual exploitation content, content involving sexualized minors, fraud or scams, unlicensed gambling, and services designed to evade law enforcement or platform abuse controls.

---

## 6. Enforcement

plek.je may investigate, throttle, suspend, restrict, remove content, disable functionality, or terminate service for AUP violations as described in the [Terms of Service](tos.md).

Reports of illegal content, abuse, or rights infringement should be sent to **abuse@plek.je**.
